...
export default function AdminUsersPage() {
  ...
  const handleSaveEdit = async () => {
    if (!editModal) return
    if (!editForm.fullName.trim()) { toast.error('Vui lòng nh?p h? tên'); return }
    setSaving(true)
    try {
      await usersApi.updateAdmin(editModal.id, editForm)
      toast.success('Ðã c?p nh?t tài kho?n')
      setEditModal(null)
      loadUsers()
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.success('Ðã xóa tài kho?n')
      setDeleting(null)
      loadUsers()
    } catch (err: any) {
      toast.error(err?.response?.data?.message || 'Xóa th?t b?i')
    }
  }
  ...
  const toggleActive = async (id: string) => {
    try {
      ...
      toast.success('Ðã c?p nh?t tr?ng thái tài kho?n')
    } catch {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-white">Qu?n lý Khách hàng</h1>
          <p className="text-brand-muted text-sm">
            {users.length} tài kho?n · {users.filter(u => u.isActive).length} dang ho?t d?ng
          </p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-brand-border bg-brand-dark/50">
                <th className="text-left px-4 py-3 text-xs text-brand-muted uppercase tracking-wide font-semibold">Ngu?i dùng</th>
                <th className="text-left px-4 py-3 text-xs text-brand-muted uppercase tracking-wide font-semibold hidden md:table-cell">S? ÐT</th>
                ...
                <th className="text-left px-4 py-3 text-xs text-brand-muted uppercase tracking-wide font-semibold hidden lg:table-cell">Ngày dang ký</th>
                <th className="text-left px-4 py-3 text-xs text-brand-muted uppercase tracking-wide font-semibold">Tr?ng thái</th>
                <th className="text-right px-4 py-3 text-xs text-brand-muted uppercase tracking-wide font-semibold">Thao tác</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="py-12 text-center text-brand-muted text-sm">Ðang t?i...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={6} className="py-12 text-center text-brand-muted text-sm">Không tìm th?y ngu?i dùng</td></tr>
              ) : filtered.map(user => (
                <tr key={user.id} className="border-b border-brand-border/40 hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-xl flex items-center justify-center text-slate-900 text-xs font-bold shrink-0"
                  ...
                  <td className="px-4 py-3 hidden md:table-cell text-slate-900 text-sm">{user.phone || '—'}</td>
                  <td className="px-4 py-3 hidden sm:table-cell">
                    <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${user.role === 'admin' ? 'text-brand-gold bg-brand-gold/10 border-brand-gold/20' : 'text-slate-900 bg-white/5 border-white/10'}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${user.isActive ? 'text-green-400 bg-green-400/10 border-green-400/20' : 'text-red-400 bg-red-400/10 border-red-400/20'}`}>
                      {user.isActive ? 'Ho?t d?ng' : 'Ðã khóa'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-brand-card border border-brand-border rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-brand-border">
              <h2 className="font-semibold text-white">?? Ch?nh s?a tài kho?n</h2>
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center gap-3 p-3 bg-brand-dark rounded-xl">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-slate-900 text-sm font-bold shrink-0"
              ...
              <div>
                <label className="text-xs text-brand-muted mb-1.5 block">H? tên *</label>
                <input value={editForm.fullName} onChange={setEF('fullName')} className="input-dark" placeholder="H? và tên" />
              </div>
              <div>
                <label className="text-xs text-brand-muted mb-1.5 block">S? di?n tho?i</label>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-white">Tài kho?n dang ho?t d?ng</span>
              </label>

              <div className="flex gap-3 pt-1">
                <button onClick={handleSaveEdit} disabled={saving}
                  className="btn-primary flex-1 flex items-center justify-center gap-2 py-3 disabled:opacity-60">
                  <CheckCircle size={15} /> {saving ? 'Ðang luu...' : 'Luu thay d?i'}
                </button>
                <button onClick={() => setEditModal(null)} className="btn-outline px-6">H?y</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {deleting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-brand-card border border-red-500/30 rounded-2xl p-6 max-w-sm w-full text-center shadow-2xl">
            ...
            <h3 className="text-white font-semibold mb-2">Xác nh?n xóa tài kho?n?</h3>
            <p className="text-brand-muted text-sm mb-5">Tài kho?n và toàn b? d? li?u liên quan s? b? xóa vinh vi?n.</p>
            <div className="flex gap-3">
              ...
              <button onClick={() => setDeleting(null)} className="flex-1 btn-outline py-2.5 text-sm">H?y</button>
            </div>
          </div>
        </div>
      )}
