...
const EMPLOYER_STATUS_OPTIONS = [
  { value: 'reviewing', label: 'Ðang xem xét', icon: Clock, color: 'text-blue-400 border-blue-400/30 bg-blue-400/5' },
  { value: 'approved', label: 'Phê duy?t', icon: CheckCircle, color: 'text-green-400 border-green-400/30 bg-green-400/5' },
...
export default function EmployerApplicationsPage() {
  ...
  const handleStatusChange = async (appId: string, status: string) => {
    ...
    try {
      ...
      toast.success('Ðã c?p nh?t tr?ng thái')
    } catch (err: any) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-white">H? so ?ng viên</h1>
        <p className="text-brand-muted text-sm">{apps.length} h? so dã nh?n</p>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Tìm theo tên, email ho?c tên v? trí..."
      ...
      {loading ? (
        ...
        <div className="card-dark p-12 text-center">
          ...
          <p className="text-white font-semibold mb-1">{search ? 'Không tìm th?y k?t qu?' : 'Chua có h? so nào'}</p>
          <p className="text-brand-muted text-sm">H? so s? xu?t hi?n t?i dây khi ?ng viên n?p vào các tin tuy?n d?ng c?a b?n.</p>
        </div>
      ) : (
                ...
                {isOpen && (
                  <div className="border-t border-brand-border p-4 space-y-3 bg-brand-dark/30">
                    ...
                    {app.status !== 'withdrawn' && (
                      <div>
                        <p className="text-brand-muted text-xs mb-2">C?p nh?t tr?ng thái</p>
